...
def viewer(request):
    if request.user.is_superuser:
        ...
        return render(request, 'viewer/mturker.html', {})
...
def getAnnotations(request):
    ...
    if request.GET.get('use_expert_annotations', False) == 'true':
        use_expert_annotations = True
    else:
        use_expert_annotations = False

    if use_expert_annotations:
        request.session['viewing_answer']

    # in Mturk view, get expert annotations 
    if use_expert_annotations:
        username = "expert"
    else:
